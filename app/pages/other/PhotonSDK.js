...
export var DemoClass = (function (_super) {
    __extends(DemoLoadBalancing, _super);
    const protocolVal = window.location.protocol === "https:"?Photon.ConnectionProtocol.Wss:Photon.ConnectionProtocol.Ws;
    function DemoLoadBalancing(DemoAppId, DemoAppVersion, updateParent, updateButton, receiveInfo) {
        var _this = _super.call(this, protocolVal, DemoAppId, DemoAppVersion) || this;
    ...
    DemoLoadBalancing.prototype.onEvent = function (code, content, actorNr) {
        this.receiveInfo(code, content, actorNr)
    };
    ...
    DemoLoadBalancing.prototype.updateRoomInfo = function () {
        this.updateRoomButtons();
        this.updateParent(this.myRoom(), this.myRoomActors());
    };
    ...
    DemoLoadBalancing.prototype.onRoomList = function (rooms) {
        ...
        for (var i = 0; i < rooms.length; ++i) {
            ...
            const nameStr = r.name.split('___')[0];
            const optName = nameStr.split('__').join(' ');
    ...
    DemoLoadBalancing.prototype.output = function (str) {
        console.log(str);
    };
    DemoLoadBalancing.prototype.updateRoomButtons = function () {
        const disableCreate = !(this.isInLobby() && !this.isJoinedToRoom());
        const disableJoin = !(this.isInLobby() && !this.isJoinedToRoom() && this.availableRooms().length > 0);
        const disableLeave = !(this.isJoinedToRoom());
        this.updateButton(disableCreate, disableJoin, disableLeave);
    };
